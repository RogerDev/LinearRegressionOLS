...
EXPORT OLS(DATASET(NumericField) X=empty_data,
  ...
  /**
    ...
    * The 'number' field of the returned NumericField records is not used and is set to 1.
  ...
  EXPORT DATASET(NumericField) Betas(DATASET(Layout_Model) model=GetModel) := FUNCTION
    ...
    // Return betas distributed by work item so that the analytics will be balanced across
    // nodes.
    return DISTRIBUTE(nfBetas, wi);
  END;
  ...
  CompRec0 make_comp0(Y_stats le, Yhat_stats ri) := TRANSFORM
    ...
    SELF         := le;
  END;
  Ycomp0 := JOIN(Y_stats, Yhat_stats, LEFT.wi=RIGHT.wi AND LEFT.number=RIGHT.number, make_comp0(LEFT, RIGHT), LOCAL);
  ...
  SHARED Ycomp  := JOIN(Ycomp0, YYhat_stats, LEFT.wi=RIGHT.wi and LEFT.number=RIGHT.number, make_comp(LEFT, RIGHT), LOCAL);
  ...
  SHARED Y_stats2 := JOIN(Y_stats1, fieldCounts, LEFT.wi=RIGHT.wi, TRANSFORM(tmpRec, SELF.K:=RIGHT.K, SELF := LEFT), LOOKUP);
  ...
  EXPORT AnovaRec getResult(tmpRec le) :=TRANSFORM
    k   := le.K;
    SST := le.var*(le.countval - 1);
    SSM := SST*le.RSquared;
    SELF.wi       := le.wi;
    SELF.Total_SS := SST;
    SELF.Model_SS := SSM;
  ...
  /**
    ...
    * - Total_SS -- Total Sum of Squares (SS) variance
    ...
    *
    * @return  DATASET(AnovaRec), one per work-item
    *
    */
  EXPORT Anova := FUNCTION
    a := PROJECT(Y_stats2, getResult(LEFT));
    return DISTRIBUTE(a, wi);
  END;
  ...
  Layout_Cell scale_vc(Layout_Cell l, Anova r) := TRANSFORM
    ...
    SELF   := l;
  END;
  mXTXinv_dist := DISTRIBUTE(mXTXinv, wi_id);
  mCoef_covar := JOIN(mXTXinv_dist, Anova, LEFT.wi_id = RIGHT.wi, scale_vc(LEFT, RIGHT), LOCAL);
  ...
  SHARED fas :=   ML_Core.FieldAggregates(X).Simple;
  
  SHARED NumericField calc_sErr(NumericField lr) := TRANSFORM
    SELF.value := SQRT(lr.value);
    SELF.number    := 1; // Number is not used and set to one for consistency
    SELF       := lr;
  END;
  ...
  EXPORT DATASET(NumericField) SE := FUNCTION
    sErr := PROJECT(Coef_covar(number=id), calc_sErr(LEFT));
    return DISTRIBUTE(sErr, wi);
  END;
  ...
  EXPORT DATASET(NumericField) TStat := JOIN(Betas(), SE, LEFT.wi = RIGHT.wi AND
                                    LEFT.id = RIGHT.id, 
                                    tStat_transform(LEFT, RIGHT), LOCAL);
  ...
  EXPORT RangeVec := RECORD
    ...
    t_FieldReal P;
  END;
  SHARED PI := 14159265358979;

  EXPORT DistributionBase := MODULE,VIRTUAL
  ...
  EXPORT TDistribution(t_Discrete v_in,t_Count NRanges = 1000) := MODULE(DistributionBase)
    SHARED v := IF(v_in > 100, 100, v_in);
    SHARED Multiplier := IF ( v & 1 = 0, Math.DoubleFac(v-1)/(2*SQRT(v)*Math.DoubleFac(v-2))
                             , Math.DoubleFac(v-1)/(Pi*SQRT(v)*Math.DoubleFac(v-2)));
    ...
    EXPORT t_FieldReal Cumulative(t_FieldReal RH) :=FUNCTION // Cumulative probability at stated point
      ...
      // cumulative(RH) = cumulative(v.RangeHigh) - prob(RH <= x < v.Rangehigh)
      // prob(RH <= x < v.Rangehigh) =  Density((RH + Rangehigh)/2) * (RH - Rangehigh) [Rectangle Rule for integration]
      InterC(RangeVec rv) := IF ( RH=rv.RangeHigh, rv.P, rv.P - Density((rv.RangeHigh+RH)/2)*(rv.RangeHigh - RH));
      RETURN MAP( RH >= MAX(cv,RangeHigh) => 1.0,
                RH <= MIN(cv,RangeLow) => 0.0,
                InterC(cv(RH>RangeLow,RH<=RangeHigh)[1]) );
    END;
  END;
  // F Distribution
  EXPORT FDistribution(t_Discrete d1_in, t_Discrete d2_in, t_Count NRanges = 1000) := MODULE(DistributionBase)
    SHARED REAL8 d1 := IF(d1_in > 100, 100, d1_in);
    SHARED REAL8 d2 := IF(d2_in > 100, 100, d2_in);
    EXPORT Low := 0;
    EXPORT HIGH := 15;
    EXPORT RangeWidth := (high - low)/NRanges;
    // Method 1 -- Wikipedia
    SHARED REAL8 Multiplier :=  POWER(d1/d2, d1/2) / Math.Beta(d1/2, d2/2);
    EXPORT t_FieldReal Density(t_FieldReal RH) := Multiplier * POWER(RH, d1/2 - 1) * 
                                               POWER(1 + d1 * RH / d2, -(d1 + d2)/2);
  ...
  E := 2.718281828459;
  // Normal Distribution
  EXPORT NormalDistribution := MODULE(DistributionBase)
   EXPORT t_FieldReal Density(t_FieldReal RH) := 1/SQRT(2*PI) * POWER(E, -RH*RH/2);
  END; 
  ...
  /**
    ...
    *         Y intercept, 2 being the coefficient for the first feature, etc. 
    */
  EXPORT pVal := JOIN(tStat, Anova, LEFT.wi = RIGHT.wi, calc_P(LEFT, RIGHT), LOCAL);
  ...
  /**
    ...
    * @return DATASET(ConfintRec) with one record per coefficient per work-item.
    *         The 'idr' field is the coefficient number, with 1 being the
    *         Y intercept, 2 being the coefficient for the first feature, etc. 
    */
  EXPORT ConfInt(Types.t_fieldReal level) := FUNCTION
    ...
    margins := JOIN(SE, Anova, LEFT.wi = RIGHT.wi, 
                     calc_margins(LEFT, RIGHT, p_level), LOCAL);
    // Use the margins computed above and the Betas themselves to compute the intervals
    RETURN JOIN(Betas(), margins, LEFT.wi = RIGHT.wi AND LEFT.id = RIGHT.id AND LEFT.id = RIGHT.id, 
                     confint_transform(LEFT,RIGHT), LOCAL);
  END;
  ...
  /**
    ...
    *
    * @return DATASET(FTestRec), one record per work-item.
    */    
